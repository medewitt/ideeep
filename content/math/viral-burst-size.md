...
## Nine viruses across the cellular parameter space
...
Eight of the nine below are RNA viruses; the last, variola (smallpox), is a large double-stranded DNA poxvirus — together they span the parameter space along all three axes at once, burst size, mutation rate, and the length of the production window.
Seven leave the cell by **budding** — HIV, influenza, and hantavirus continuously; measles, mumps, and RSV while fusing the cell into dying syncytia; dengue before the cell apoptoses — while poliovirus packages progeny internally and releases them in a single **lytic burst**, and variola builds virions in cytoplasmic factories and is likewise freed mostly when the cell dies.
...
![Left: cumulative virions produced by one cell over time for all nine viruses — poliovirus bursts fastest, influenza builds fast then lyses, measles, mumps and RSV bud into dying syncytia, dengue buds then the cell apoptoses, variola builds in cytoplasmic factories then is freed on lysis, HIV reaches a large yield before the cell dies, and hantavirus produces slowly but never lyses, so its window stays open. Right: the nine viruses on the burst-size / mutation-rate plane, with variola sitting far below the RNA viruses in mutation rate, against diagonals of constant per-cell mutational output B times mu.](../assets/figures/burst-three-viruses.svg "fig:threevirus")
...
**Production mode sorts their stochastic fate.** The seven budding viruses release virus continuously rather than in a synchronized burst, so the offspring distribution of a single infected cell is closer to the over-dispersed geometric of [@eq:qcont] than to the Poisson of [@eq:qburst], with extinction probability near $1/R_0$.
...
Poliovirus makes the mechanism visible: because it releases its whole burst at lysis, a single cell's successful-offspring count is Poisson [@eq:qburst] rather than geometric, so at the *same* mean $R_0$ a lone poliovirus-infected cell is meaningfully **harder** to extinguish than any of the budding seven — the burst-versus-continuous gap of [@fig:extinction], now a difference between real viruses.
...
**Mutational output separates the fast evolvers from the stable ones.** The per-cell mutational output is $B\mu$, and the right panel of [@fig:threevirus] places all nine viruses on the $B$–$\mu$ plane against diagonals of constant $B\mu$.
...
Poliovirus and measles are the cautionary corner: poliovirus has the *highest* per-cell mutational output of the nine ($B\mu \approx 6$, a large burst copied at RdRp fidelity) and measles carries an ordinary RNA-range $\mu$, yet both are antigenically almost frozen — poliovirus in three fixed serotypes, measles as a single serotype the 1960s vaccine still matches — because their capsid and glycoprotein structures are functionally constrained, so most of that variation is either lethal or invisible to immunity.
Mumps is the instructive foil to its close relative measles: near-identical $B$ and $\mu$, the same syncytial paramyxovirus biology, yet its surface proteins are *not* as locked down, so the same mutational supply has produced twelve genotypes and repeated vaccine-era outbreaks in highly immunised populations — a reminder that whether variation becomes useful is a property of the protein, not of $B\mu$.
