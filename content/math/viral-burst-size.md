...
**Lytic** viruses — the bacteriophage archetype — accumulate progeny internally and release them in one burst when the cell lyses, so $B$ and $L$ are tightly coupled.
...
The split is cleaner in theory than in cells: careful single-cell work now argues that [nonlytic egress dominates the animal-virus world](https://doi.org/10.1146/annurev-biochem-052521-120140), with even textbook "lytic" nonenveloped viruses such as poliovirus exported largely in membrane vesicles hours *before* the cell dies — so read the lytic burst as a mathematical idealization (exact for phages, approximate for animal viruses) rather than a literal account of how progeny leave the cell.
...
> Both idealizations also assume virions establish *independently*; when a virus egresses [en bloc — many genomes bundled inside one membrane vesicle](https://doi.org/10.1146/annurev-biochem-052521-120140), transmission turns collective and the multiplicity of infection climbs, a further departure from the single-genome offspring counts above.
...
| **Poliovirus** | +ssRNA, non-segmented, ~7.5 kb | **lytic burst** (much egress nonlytic, in EVs) | lyses by ~6–8 h, but egress precedes it | ~2–3 h | ~$10^4$–$10^5$ per cell | ~$2\times10^{-4}$ (RdRp, no proofreading) | high diversity but 3 fixed serotypes; vaccine-derived reversion |
...
| **Variola** | **dsDNA**, ~186 kb | on lysis (MV) plus budding (EV) | dies (cytopathic, ~1–2 days) | ~6–12 h | ~$10^3$–$10^4$ | ~$10^{-6}$ (**proofreading** DNA pol) | genetically stable; **eradicated** by one vaccine |
...
Poliovirus is the same accounting compressed into hours: it packages $10^4$–$10^5$ virions in a cell that lasts only about 6–8 h — a very high production rate over a very short window, the steep early climb in the left panel.
...
Variola sits on the same side of that gap: much of its progeny is freed when the cell lyses rather than trickled out over a lifetime, so its single-cell offspring count leans burst-like too, a Poisson-ish case set against the geometric budders.
Both "burst" cases carry the caveat flagged earlier, though: poliovirus and the poxviruses in fact export a large share of their yield nonlytically, in vesicles and often en bloc, so their true offspring statistics fall *between* the Poisson and geometric extremes rather than sitting cleanly at the burst end — the diagram treats them as the idealized burst case to make the mechanism legible, not as a literal claim that they only release at lysis.
