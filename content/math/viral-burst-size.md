...
## Eight viruses across the cellular parameter space
...
Seven of the eight below are RNA viruses; the last, variola (smallpox), is a large double-stranded DNA poxvirus — together they span the parameter space along all three axes at once, burst size, mutation rate, and the length of the production window.
Six leave the cell by **budding** — HIV, influenza, and hantavirus continuously; measles and RSV while fusing the cell into dying syncytia; dengue before the cell apoptoses — while poliovirus packages progeny internally and releases them in a single **lytic burst**, and variola builds virions in cytoplasmic factories and is likewise freed mostly when the cell dies.
Those differences in how, how many, and how fast explain a great deal of their epidemiology.

| Trait | HIV-1 | Influenza A | Hantavirus | Poliovirus | Measles | RSV | Dengue | Variola |
|-------|-------|-------------|------------|------------|---------|-----|--------|---------|
| Genome | retrovirus, +ssRNA → DNA, ~9.7 kb | −ssRNA, 8 segments, ~13.5 kb | −ssRNA, 3 segments, ~12 kb | +ssRNA, non-segmented, ~7.5 kb | −ssRNA, non-segmented, ~15.9 kb | −ssRNA, non-segmented, ~15.2 kb | +ssRNA, non-segmented, ~10.7 kb | **dsDNA**, ~186 kb |
| Production mode | budding, continuous | budding, ~continuous | budding, continuous | **lytic burst** | budding, syncytial | budding, syncytial | budding (secretory) | released mainly on lysis |
| Infected-cell fate | dies in ~1–2 days (cytopathic + CTL) | dies in ~1 day (cytopathic) | **survives — non-cytopathic, persistent** | lysed in ~6–8 h (rapid host shutoff) | fuses into dying syncytia over ~2–3 days | fuses into dying syncytia (~2–3 days) | dies by apoptosis (~2–3 days) | dies (cytopathic, ~1–2 days) |
| Eclipse $\tau_E$ | ~18–24 h | ~6 h | ~1–3 days | ~2–3 h | ~12–24 h | ~10–12 h | ~12–24 h | ~6–12 h |
| Burst size $B$ | ~$10^4$–$10^5$ total virions, only ~1 in $10^2$–$10^3$ infectious | ~$10^2$–$10^4$ (single-cell mean ~350–700) | low–moderate, IFN-limited, sustained | ~$10^4$–$10^5$ per cell | low–moderate (~$10^2$–$10^3$) | low; high particle-to-PFU ratio | ~$10^3$–$10^4$ | ~$10^3$–$10^4$ |
| Per-site mutation $\mu$ | ~$2.4\times10^{-5}$ (RT); up to ~$4\times10^{-3}$ in vivo via APOBEC | ~$1.8$–$2.5\times10^{-4}$ (2–3 per genome) | RNA-range but constrained; low diversity | ~$2\times10^{-4}$ (RdRp, no proofreading) | ~$10^{-4}$ (RNA-range) | ~$10^{-4}$ (RNA-range) | ~$1$–$2\times10^{-4}$ (RdRp) | ~$10^{-6}$ (**proofreading** DNA pol) |
| Evolutionary signature | quasispecies, rapid drug/immune escape | antigenic drift + segment reassortment | host codivergence, spillover dead-end | high diversity but 3 fixed serotypes; vaccine-derived reversion | **monotypic** — one serotype, antigenically stable | A/B subgroups; variable G, conserved F | 4 serotypes; cross-serotype ADE | genetically stable; **eradicated** by one vaccine |

![Left: cumulative virions produced by one cell over time for all eight viruses — poliovirus bursts fastest, influenza builds fast then lyses, measles and RSV bud into dying syncytia, dengue buds then the cell apoptoses, variola builds in cytoplasmic factories then is freed on lysis, HIV reaches a large yield before the cell dies, and hantavirus produces slowly but never lyses, so its window stays open. Right: the eight viruses on the burst-size / mutation-rate plane, with variola sitting far below the RNA viruses in mutation rate, against diagonals of constant per-cell mutational output B times mu.](../assets/figures/burst-three-viruses.svg "fig:threevirus")
...
Poliovirus makes the mechanism visible: because it releases its whole burst at lysis, a single cell's successful-offspring count is Poisson [@eq:qburst] rather than geometric, so at the *same* mean $R_0$ a lone poliovirus-infected cell is meaningfully **harder** to extinguish than any of the budding six — the burst-versus-continuous gap of [@fig:extinction], now a difference between real viruses.
...
**Mutational output separates the fast evolvers from the stable ones.** The per-cell mutational output is $B\mu$, and the right panel of [@fig:threevirus] places all eight viruses on the $B$–$\mu$ plane against diagonals of constant $B\mu$.
...
Variola is the opposite extreme, and the one point that leaves the RNA cloud entirely: its proofreading DNA polymerase holds the per-site rate near $10^{-6}$, an order of magnitude below any RNA virus here, so even a burst of $\sim\!10^4$ gives the *lowest* per-cell mutational output on the plane ($B\mu \approx 0.015$) — the molecular reason smallpox stayed a single, antigenically fixed target for centuries and could be driven to global eradication with one vaccine.
They are the standing reminder that $B\mu$ sets the *supply* of variants, not whether any of them can become useful: read through that product, the same quantity that sets a phage's supply of escape mutants explains why influenza and HIV are moving targets, why hantavirus (little variation) and poliovirus and measles (plenty of variation, nowhere to go) are not, why RSV and dengue land in between, and why variola — little variation *and* structurally constrained — was the one virus a vaccine could corner into extinction.
