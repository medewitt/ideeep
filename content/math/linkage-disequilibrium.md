...
![Left: recombination erodes LD geometrically, Dₜ = D₀(1−c)ᵗ, so tightly linked loci (small c) retain disequilibrium for many generations while unlinked loci (c = 0.5) lose half of any remaining D each generation. Right: in the worked example the observed haplotype frequencies depart from the product of allele frequencies expected under independence, and that gap is D = 0.40 − 0.25 = 0.15 (giving r² = 0.36).](../assets/figures/linkage-disequilibrium.svg "fig:ld")
